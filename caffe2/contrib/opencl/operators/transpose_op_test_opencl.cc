...
void load_n_run(Workspace &ws, const DeviceType dt)
{
  NetDef init, predict;
...
void compare_tensors_by_name(const std::string &t_expected, const std::string &t_result, Workspace &ws_opencl, OpenCLContext &context) {
  ...
  auto cl_expected_tensor = ws_opencl.GetBlob(t_expected)->Get<TensorCL>().Clone();
  ...
  auto cl_output_tensor = ws_opencl.GetBlob(t_result)->Get<TensorCL>().Clone();
