...
void OpenCLContext::CopyBytes<OpenCLContext, CPUContext>(size_t nbytes, const void *src, void *dst) {
  cl::copy(queue(), *((cl::Buffer*)src), static_cast<char*>(dst), static_cast<char*>(dst) + nbytes);
}
...
void OpenCLContext::CopyBytes<CPUContext, OpenCLContext>(size_t nbytes, const void *src, void *dst) {
  ...
  cl::Event event;
  queue().enqueueWriteBuffer(*((cl::Buffer*)(dst)), CL_FALSE, 0, nbytes, static_cast<const char*>(data), nullptr, &event);
