...
// float16 is data structure from caffe2 and it isn't directly
// supported by OpenCL without additional converters
//DELEGATE_SIMPLE_OPENCL_BINARY_INFIX_FUNCTION(float16, Add, +);
//DELEGATE_SIMPLE_OPENCL_BINARY_INFIX_FUNCTION(float16, Sub, -);
//DELEGATE_SIMPLE_OPENCL_BINARY_INFIX_FUNCTION(float16, Mul, *);
//DELEGATE_SIMPLE_OPENCL_BINARY_INFIX_FUNCTION(float16, Div, /);
...
DELEGATE_SIMPLE_UNARY_FUNCTION(float, InvSqrt, Q_rsqrt)
...
{
  auto kernel = context->BuildKernelCached(kFillWithNaN);
  ...
  size_t type_size)
{
  ...
  auto kernel1 = context->BuildKernelCached(kFillWithNaN);
  ...
  auto kernel2 = context->BuildKernelCached(kRowwiseMax);
...
static constexpr const char* kMatrixMulOperatorANBNCN = R"CLC(
  ...
  __kernel void K(const int M, const int N, const int K,
    ...
    OpenCLContext* context)
{
  ...
  auto kernel = context->BuildKernelCached(kernel_src);
...
static constexpr const char* kMatrixMulOperatorBatchedANBNCN = R"CLC(
  // First naive implementation
  __kernel void K(const int batch_size,
...
void Set<float, caffe2::OpenCLContext>(unsigned long size, float value,
  ...
  auto kernel1 = context->BuildKernelCached(kSetKernel);
  ...
  size_t type_size)
{
  ...
  //Now, as all output values were initialized, we can begin to search for Max per column:
  auto kernel = context->BuildKernelCached(kColwiseMax);
  size_t in_mem_size = N_rows * D_cols * type_size;
  auto mat_x_in = context->New(in_mem_size);
...
void Transpose<cl::Buffer, OpenCLContext>(
  ...
  case 1:
    ...
    kernel = context->BuildKernelCached(kTranspose1D);
    break;
  case 2:
    problem_range = cl::NDRange(x_dims[0], x_dims[1]);
    kernel = context->BuildKernelCached(kTranspose2D);
    break;
  case 3:
    problem_range = cl::NDRange(x_dims[0], x_dims[1], x_dims[2]);
    kernel = context->BuildKernelCached(kTranspose3D);
    break;
  case 4:
    problem_range = cl::NDRange(x_dims[0] * x_dims[1], x_dims[2], x_dims[3]);
    kernel = context->BuildKernelCached(kTranspose4D);
    break;
  default:
...
kernel void K(const __global float* in_mat,
              const int N_rows,
              __global float* out)
{
  *out = 0;
...
static void runSumKernel(
  ...
  size_t type_size)
{
  auto kernel = context->BuildKernelCached(kSum);
...
DATA_TYPE mul(DATA_TYPE A, DATA_TYPE B)
{
 ...
 /**
  * OpenCL is not able to synchronize work-items in different work-groups.
  ...
  */
kernel void K( int size, global DATA_TYPE *x, global DATA_TYPE *y)
{
...
void Funcname<T, OpenCLContext>(                                                \
  ...
  const T* x,                                                                   \
  T *y,                                                                         \
  Tensor<OpenCLContext>* scratch_ptr,                                           \
  OpenCLContext* context                                                        \
  ) {                                                                           \
...
void Funcname<T, OpenCLContext>(                                                \
  ...
  OpenCLContext* context                                                        \
    ...
    ReduceKind(N, x, z, static_cast<Tensor<OpenCLContext> *>(nullptr), context);\
  }
...
kernel void K(const int N_rows,
              const __global float* A,
              const __global float* B,
              __global float* out)
{
...
static void runDotKernel(
  ...
  size_t type_size)
{
  auto kernel = context->BuildKernelCached(kDot);
  size_t in_mem_size = N_rows * type_size;
...
static void runScaleKernel(
  ...
  size_t type_size)
{
  auto kernel = context->BuildKernelCached(kScale);
